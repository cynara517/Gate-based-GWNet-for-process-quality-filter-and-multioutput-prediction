...
class graph_constructor(nn.Module):
    ...
    def forward(self, idx):
        ...
        adj = adj*mask
        #试着打印
        graph_data2 = adj.detach().cpu().numpy()
        with open('adj_6.pkl', 'wb') as f:
            pickle.dump(graph_data2, f)
        return adj
    ...
    def fullA(self, idx):
        ...
        adj = F.relu(torch.tanh(self.alpha*a))
        #试着打印
        graph_data3 = adj.detach().cpu().numpy()
        with open('adj_layer_fullA.pkl', 'wb') as f:
            pickle.dump(graph_data3, f)
        return adj
...
class graph_global(nn.Module):
    ...
    def forward(self, idx):
        #试着打印，这里不知道会不会是一个很大的图
        graph_data4 = F.relu(self.A).detach().cpu().numpy()
        with open('adj_layer_graph_global.pkl', 'wb') as f:
            pickle.dump(graph_data4, f)
...
class graph_undirected(nn.Module):
    ...
    def forward(self, idx):
        ...
        adj = adj*mask
        # 试着打印无向图
        graph_data5 = adj.detach().cpu().numpy()
        with open('adj_layer_undirected.pkl', 'wb') as f:
            pickle.dump(graph_data5, f)
        return adj
...
class graph_directed(nn.Module):
    ...
    def forward(self, idx):
        ...
        adj = adj*mask
        # 试着打印有向图
        graph_data6 = adj.detach().cpu().numpy()
        with open('adj_layer_directed.pkl', 'wb') as f:
            pickle.dump(graph_data6, f)
        return adj
